  ...
  const renderBeds = (capacity, available) => {
    ...
    for (let i = 0; i < capacity; i++) {
      beds.push(
        <div
          key={i}
          className={`p-2 rounded-lg flex-1 flex justify-center border backdrop-blur-sm shadow-inner ${i < occupied ? "bg-[#0b1118]/80 border-white/10" : "bg-emerald-500/20 border-emerald-500/30"}`}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className={`h-6 w-6 drop-shadow-md ${i < occupied ? "text-slate-600" : "text-emerald-400"}`}
            fill="currentColor"
            viewBox="0 0 24 24"
          >
            <path d="M20 9.557V3h-2v2H6V3H4v6.557C2.81 10.25 2 11.525 2 13v4h2v-2h16v2h2v-4c0-1.475-.81-2.75-2-3.443zM18 7H6v2h12V7zM4 13c0-.552.449-1 1-1h14c.552 0 1 .448 1 1v2H4v-2z" />
          </svg>
        </div>,
      );
    }
    ...
    <div className="space-y-6 max-w-5xl mx-auto animate-fadeIn">
      <div className="flex items-center justify-between border-b border-white/10 pb-6 bg-[#15202b]/40 p-6 rounded-2xl backdrop-blur-md shadow-lg">
        <div className="flex items-center gap-4">
          <button
            onClick={() => navigate(-1)}
            className="group p-2.5 bg-[#0b1118]/60 hover:bg-white/10 rounded-xl border border-white/10 transition-all backdrop-blur-md"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        {rooms.map((room) => (
          ...
          >
            <div className="flex justify-between items-start">
              ...
              <div
                className={`px-2.5 py-1 rounded-md text-xs font-bold border backdrop-blur-md ${room.available === 0 ? "bg-red-500/20 text-red-300 border-red-500/30" : "bg-[#137fec]/20 text-[#3b9cff] border-[#137fec]/30"}`}
              >
